...
my $content_line;
open LOL,"</www/wwwroot/dracp/tools/tools_result_static_durg.html" or die "Counldn't open: $!";
	while (my $line = <LOL>){
		$content_line .= $line;
	}
close LOL;
...
if(-e "../tmp/simi_search_tmp_drug/tmp.out"){
	...
	foreach $edit(@my_line){
		...
		$edit =~  s/BAPD(\d+)/<a href\=\"http:\/\/dracp\.cpu-bioinfor.org\/browse\/content.php\?id=BAPD$1\"\>BAPD$1\<\/a\>/ if $edit =~ /BAPD(\d+)/;
